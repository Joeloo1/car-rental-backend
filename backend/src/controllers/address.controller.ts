...
// Get all Address
export const getAllAddress = catchAsync(async (req: AuthRequest, res: Response) => {
...
// Get Address By ID
export const getAddress = catchAsync(async (req: AuthRequest, res: Response) => {
  const address = await GetAddressByIdService(req.user!.id, req.params.id as string);
...
export const updateAddress = catchAsync(async (req: AuthRequest, res: Response) => {
  const address = await UpdateAddressService(req.user!.id, req.params.id as string, req.body);

  logger.info(`User with ID: ${address.userId} updating address with ID: ${address.id}`);
  res.status(200).json({
    status: "success",
